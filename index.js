...
function getName(newobject) {
	/* code here */
	const named = newobject.name;
	return `Hello, my name is ${named}`;
}
...
function getCarInfoByIndex(inventory, index) {
	/* code here */
	const the4th = inventory.find((item, index2) => {
		return index2 === index;
	});
	return `The car is a ${the4th.car_make} ${the4th.car_model}`;
}
...
function getLastCarInfo(inventory) {
	/* code here */
	let poppy = inventory;
	let lastone = poppy.length - 1;
	let poppy2 = poppy[lastone];
	return `This is a ${poppy2.car_make} ${poppy2.car_model}`;
}
 ...
 * it will return `This is a Lincoln Navigator`.
 */
function getCarInfoById(inventory, numyid) {
	/* code here */
	var carinfo = inventory.find((item, index2) => {
		return index2 === numyid;
	});
...
function sortCarInventory(inventory) {
	/* code here */
	const carInv = inventory.sort(function(a, b) {
...
function getModelYears(inventory) {
	...
	const carInv2 = inventory.sort(function(item) {
		if (item.car_year) {
			carInz.push(item.car_year);
			return -1;
		}
		return 0;
	});
 ...
 * }
 */
//let sum = (a, b) => a + b; // code here!
//let addFive = num => num + 5; // code here!
//let argTimesTwo = num => num * 2; // code here!
